...
#### choosing to split on 7
pruned_tree7 <- prune.tree(regression_tree_model, best = 7)
summary(pruned_tree7)
plot(pruned_tree7)
text(pruned_tree7, pretty = 0, cex = 0.7, font = 2)

pred <- predict(pruned_tree7)
SSres <- sum((crime_data$Crime - pred)^2)
SStot <- sum((crime_data$Crime - mean(crime_data$Crime))^2)
R2 <- 1 - SSres / SStot
R2
...
predict(pruned_tree7, newdata = test_data)
